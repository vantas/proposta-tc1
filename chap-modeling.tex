...
\item[Client] \hfill \\
    ...
    \begin{center}
    \begin{algorithmic}[1]
    \Function{HandleClientArrival}{$event$}
      \State $floor \gets floors.$\Call{Find}{$event.floor$}
      \If{client is going up}
        \If{$floor.upLine.size > 0$}
          \State $floor.upLine.$\Call{Enqueue}{event.client}
        \Else
          \State do something clever
        \EndIf
      \Else
        \If{$floor.downLine.size > 0$}
          \State $floor.downLine.$\Call{Enqueue}{event.client}
        \Else
          \State do something clever
        \EndIf
      \EndIf
    ...
    \begin{center}
    \begin{algorithmic}[1]
    \Function{HandleElevatorArrival}{$event$}
      \State $floor \gets floors.$\Call{Find}{$event.floor$}
      \State $elevator \gets elevators.$\Call{Find}{$event.elevator$}
...
\Function{HandleAnyEvent}{$event$}
  \State \Call{AdvanceTime}{$event.eventTime$}
\EndFunction
\end{algorithmic}
\end{center}
\caption
...
\item[EventDispatcher] \hfill \\
    ...
    \Function{Notify}{$event$}
      \For{\textbf{each} $observer$ \textbf{in} $observers$}
        \State $observer.$\Call{Notify}{$event$}
      \EndFor
    \EndFunction
    \end{algorithmic}
    \end{center}
