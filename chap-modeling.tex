...
e paradigmas poderíam ser aplicadas. Para o projeto do simulador de sistemas de
elevadores deste estudo, doravante chamado apenas de simulador, optou-se pelo
paradigma de \textit{Programação Orientada à Objetos}. Esta escolha se deu pelos
seguintes motivos:
...
somente na ocasião de algum evento. Sendo assim, é preciso ser possível representar um evento no contexto do simulador.
...
Durante a execução da simulação, à medida que eventos ocorrem, determinados
componentes do simulador deverão de acordo~-~i.~e.,~deverão alterar seu estado
interno de acordo com o evento ocorrido, levando o estado do simulador à uma
nova situação. Estes componentes são:
...
À seguir são apresentadas as classes que representam estes componentes no
simulador.

\subsection{Estado do sistema}

Entre os componentes fundamentais de um simulador destaca-se a representação do
\textit{estado do sistema}. Entende-se por \textit{estado do sistema} como uma
coleção de variáveis necessárias para descrever o estado do sistema em um
instante em particular da simulação \cite{Law}. Neste projeto, a classe
\textit{Building} é responsável por encapsular o conjunto de informações que
definem este estado (figura \ref{fig:diagram:model}). Esta classe é responsável
por gerenciar múltiplas instâncias de elevadores (classe \textit{Elevator}),
andares (classe \textit{Floor}) e clientes (classe \textit{Client}) e relacionar
estas instâncias entre si - reproduzindo, deste modo, as dinâmicas do sistema do
...
otimizem o tempo de espera médio. Portanto, a classe \textit{Stats} possui os
seguintes contadores:

\begin{description}
  \item[\texttt{totalWaitingTime}] \hfill \ Tempo de espera total.
  \item[\texttt{totalJourneyTime}] \hfill \ Tempo de jornada total.
  \item[\texttt{totalTime}]        \hfill \ Tempo de simulação total.
  \item[\texttt{totalPassengers}]  \hfill \ Total de passageiros atendidos.
\end{description}
...
que devem reagir na ocorrência de um evento. Porém resta as questões de como
...
se que facilmente se saiba qual será o próximo evento a ocorrer.
...
Por último mas não menos importante, é apresentado a classe
\textit{ReportBuilder}, ilustrado pela figura \ref{fig:diagram:report}. Esta
classe tem por responsabilidade a elaboração de um relatório (em arquivo) à
