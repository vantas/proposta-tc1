...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item Sua lotação atual.{\color{red} comentário para professor João Batista: a capacidade máxima é definida pela variável \textbf{C} e é uma característica da instância do problema e não uma variável que compõe o estado atual do sistema}
  \end{itemize}
\end{itemize}
...
\begin{itemize}
  ...
  \item Para cada elevador:
  \begin{itemize}
    \item A hora em que partiu do lobby.{\color{red} comentário para professor João Batista: ``somente lobby?'' sim, é a informação necessária para calcular o RTT}
  \end{itemize}
\end{itemize}
...
\begin{description}[leftmargin=!,labelwidth=\widthof{\bfseries Pu}]
  ...
  \item[D]
  Função de distribuição de probabilidade de chegada/saída de passageiros.{\color{red} comentário para professor João Batista: ``em cada piso? só no lobby?'' A chegada sempre ocorre no lobby e as saídas são as pessoas indo embora dos outros andares (em direção ao lobby)}
  \item[Pu]
  Propósito do prédio: \textit{residencial} (com baixo fluxo entre os andares superiores), \textit{comercial com múltiplas empresas} (com médio fluxo entre andares superiores) ou \textit{comercial
    com única empresa} (com alto fluxo entre andares superiores).
\end{description}
