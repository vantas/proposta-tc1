...
Seu funcionamento é trivial: o elevador mais próximo da chamada sempre
atenderá esta chamada~\cite{Friese20061908}.
...
esta chamada sem alterar a agenda de nenhum elevador. O algoritmo de
...
é considerar o sentido em que o elevador está indo para atender a
chamada~\cite{Friese20061908}. Isto implica em considerar-se agora
a informação de sentido das chamadas. É importante notar que ainda não se
considera quantas pessoas fizeram uma chamada~-~apenas sabe-se que há chamadas no
...
sentido de onde a chamada se originou. Neste caso, apenas os elevadores $E2$ e
$E3$ serão considerados. O elevador $E3$ está mais próximo da chamada, então será
escolhido.
...
\item \textbf{$p$} é o número do andar onde a chamada se originou
\item \textbf{$\lambda$} é
  \begin{list}{$\circ$}{}
...
\begin{figure}[htb!]
  ...
  \caption[Exemplo de sistema de 3 elevadores com 8 andares]{Exemplo de sistema
    de 3 elevadores com 8 andares, com uma chamada para descer a partir do oitavo
...
Sabemos que há uma chamada no oitavo andar, para descer\footnote{Na
...
de $E_{1}$ para atender a chamada no oitavo andar, e depois continuar na direção original.
...
\footnote{\textit{e.g.}, pode ser vantajoso um elevador mudar de direção para
  atender uma chamada a um andar de distância, caso a alternativa seja fazer a
...
Novamente, para esta função, o Elevador $E_{1}$ é eleito para atender a chamada.
...
elevador deve atender a próxima chamada'' (figura~\ref{fig:planning}),
ou ``qual chamada deve ser atendida por qual elevador''. % TODO: Esta ilustração
...
tomada é ``qual elevador deve atender a próxima chamada da fila?'', e, para isto,
...
elevador $E2$ atenderá a primeira e a segunda chamadas da fila, e então o
elevador $E1$ atenderá a terceira chamada, como pode ser visto pelo caminho destacado.
...
um processamento central que decide que um elevador deve atender a chamada,
temos todos os elevadores calculando por conta própria se vale a pena atender
uma chamada ou não.
...
sistema: o que um elevador deve fazer quando não há chamadas para ele.
