...
literatura pesquisada na forma de algoritmos. Tais algoritmos possuem
complexidades distintas, indo desde algoritmos triviais, onde sequer podem ser
classificados como algoritmos de Inteligência Artificial mas são interessantes
para fins de comparações, até soluções mais complexas, onde mais dados são
necessários.
...
Portanto, os algoritmos aqui descritos tentam fazer inferências a respeito de
dados que não possuem, quando relevante\footnote{\textit{e.g.} Podemos estimar a
lotação do elevador com base no peso reportado pela balança interna do elevador,
que já se encontra nele por motivos de segurança.}, ou tentar tomar decisões
ignorando os dados que não estão disponíveis.

\section{\label{sec:ai:nn}Nearest Neighbour}

O algoritmo de \textit{Nearest Neighbour}, ou \textit{Vizinho Mais Próximo}, é o
mais simples de todos e servirá como base para comparação e avaliação dos demais
algoritmos. Seu funcionamento é trivial: o elevador mais próximo do chamado
sempre atenderá este chamado, independente de quaisquer outras variáveis.

\begin{algorithm}[htb]
\begin{center}
\begin{algorithmic}[1]
\Function{NearestNeighbor}{$destination, elevatorCollection$}
  \State $nearestElevator \leftarrow null$
  \State $nearestDistance \leftarrow \infty$
  \For{\textbf{each} $elevator$ in $elevatorCollection$}
    \State $distance \leftarrow \Call{abs}{elevator.currentFloor - destination}$
    \If{$distance = 0$}
      \State \textbf{return} $elevator$
    \EndIf
    \If{$distance < nearestDistance$}
      \State $nearestElevator \leftarrow elevator$
      \State $nearestDistance \leftarrow distance$
    \EndIf
  \EndFor
  \State \textbf{return} $nearestElevator$
\EndFunction
\Statex
\Function{Abs}{value}
  \If{$value > 0$}
...
\end{center}
\caption
   {\label{alg:nearestneighbor}Nearest Neighbor}
\end{algorithm}
