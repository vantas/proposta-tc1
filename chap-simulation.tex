...
A fim de decidir de forma objetiva qual a melhor abordagem para um dado
sistema,~\cite{Law} propõe uma reflexão (figura~\ref{fig:systemstudy}) através
...
\begin{itemize}
  ...
  \textbf{Exemplo}: construir um novo módulo na Estação Espacial Internacional.
...
forma de relações lógicas e quantitativas (equações matemáticas e notações
simbólicas), são então avaliadas numericamente enquanto são coletados dados com
o intuito de observar de que forma o sistema real reagiria.
...
estimativas baseadas em observação e senso comum.
...
À partir de um modelo matemático a ser estudado por meio de simulação (doravante
...
a modelagem de um sistema na medida em que ele \textbf{evolui ao longo do tempo}
...
tempo da simulação na medida em que a simulação é executada. Também se faz
necessário um mecanismo de avanço de tempo para avançar este tempo de um valor
para o outro. Esta variável, que armazena o valor atual do tempo de simulação, é
chamada de \textit{relógio da simulação} \cite{Law}. Geralmente, não há relação
entre o tempo de simulação e o tempo necessário para a simulação ser executada.
Por exemplo, um experimento pode simular o funcionamento de um banco entre 9h e
17h (tempo de simulação), mas o tempo necessário para executar a simulação foi
de 4 minutos.
...
\textit{relógio da simulação} é inicializado em 0 (zero) e é determinado o tempo
em que ocorrerão futuros. Então, o \textit{relógio da simulação} é avançado para
o tempo da ocorrência do \textit{mais iminente} (primeiro) destes eventos
futuros. Neste ponto do tempo, o estado do modelo é atualizado de acordo com o
evento que ocorreu e novas ocorrências eventos futuros são geradas. Então,
\textit{relógio da simulação} avança novamente do (novo) \textit{mais iminente}
...
\textbf{TO-DO: O PRÓXIMO PARÁGRAFO ESTÁ MAL ESCRITO. MELHORAR!}

Na medida em que todas as alterações no estado do sistema ocorrem na ocasião da
ocorrência de algum evento, os períodos de inatividade são descartados pelo fato
do valor do \textit{relógio da simulação} pular para o tempo de cada evento,
evento após evento - i.~e.~, o tempo compreendido entre eventos, onde não
ocorreu nenhuma alteração no estado do sistema, não necessita ser computado
\cite{Sim}.

\item[Avanço de tempo de incremento fixo] \hfill

Na abordagem de \textit{avanço de tempo de incremento fixo}, o \textit{relógio
da simulação} é alterado sempre no mesmo incremento de tamanho fixo,
independente de ocorrência de eventos. A cada avanço é feita uma varredura para
verificar se haviam eventos agendados dentro daquele intervalo. Se um ou mais
eventos estavam agendados para ocorrer no decorrer do intervalo, considera-se
que sua ocorrência ocorreu no \textit{fim} do intervalo e o estado do sistema é
...
\item[Rotina de Inicialização] Subprograma para inicializar a simulação no instante de simulação 0 (zero);
...
\item[Rotina de Evento] Subprograma que modifica o estado do sistema quando um tipo particular de evento ocorre (existe uma rotina de evento para cada tipo de vento);
